...
int main()
{
    ...
    fstream fs;
    fs.open("BangTanSuat.txt", fstream::out); // Mở tệp để ghi bảng tần suất
    if (fs.is_open())
    {
        for (auto it = tansuat.begin(); it != tansuat.end(); ++it)
        {
    ...
    if (tansuat.size() == 1)
    {
        ...
        cout << "Huffman Code of each letter: " << xau[0] << ": 0\n\n";
        cout << "Huffman Code of input string:\n";
        for (int i = 0; i < xau.size(); i++)
        {
            cout << 0;
        }
